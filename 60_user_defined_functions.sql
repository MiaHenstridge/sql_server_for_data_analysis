...
CREATE FUNCTION dbo.udfPercentage(@numerator FLOAT, @denominator FLOAT)
RETURNS VARCHAR(32)
AS
	BEGIN
		RETURN FORMAT((@numerator * 1.0)/@denominator, 'p')
	END
...
SELECT
	BusinessEntityID,
	VacationHours,
	PercentOfMaxVacation = dbo.udfPercentage(VacationHours, @MaxVacationHours)
FROM AdventureWorks2019.HumanResources.Employee